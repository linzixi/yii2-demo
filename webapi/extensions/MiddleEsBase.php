...
class MiddleEsBase
{
    ...
    public function __construct($limitField = false,$scroll = false,$collapse = false){   // $limitField  后期改为false
        ...
        $this->startTime = date('Y-m-d',strtotime('-4 month'));
...
//                        "minimum_should_match" => "60%"
    ...
    public function posttime($begin_time = null,$end_time = null){
        ...
        if($begin_time && $begin_time == date('Y-m-d H:i:s',strtotime($begin_time))){
            $this->startTime = $begin_time>=$this->startTime ? $begin_time:$this->startTime;
            $timeRange['range']['news_posttime']['gte'] = $begin_time;
        }
        if($end_time && $end_time == date('Y-m-d H:i:s',strtotime($end_time))){
            $this->endTime = $end_time;
            $timeRange['range']['news_posttime']['lte'] = $end_time;
        }
...
//            ]
    ...
    public function dateGroup($type = 'day'){
        ...
        $this->condition['aggs']['group_by_date'] =
            [
                "date_histogram" => [
                    "field" => "solr_create_time", // news_postdate  没有结果，难道不是date类型？
                    "interval"=> $interval,
                    "format"=> $format,
                    "min_doc_count" => 0,
                    "extended_bounds" => [
...
//        $this->condition['aggs']['size'] = 30;
     ...
     * @return $this
     */
    public function platformName($mediaNames = []){
        $mediaNameRange = null;
        if(!empty($mediaNames)){
            if(is_string($mediaNames) || (is_array($mediaNames) && count($mediaNames) == 1)) {
                $mediaName = is_string($mediaNames) ? $mediaNames : $mediaNames[0];
                $mediaNameRange = [
                    'term' => [
                        'platform_name' => $mediaName
                    ]
                ];
            }else{
                $mediaNameRange = [
                    'terms' => [
                        'platform_name' => $mediaNames
                    ]
                ];
            }
        }
