...
class ShudiEsBase
{
    ...
    const ES_GATEWAY_URL = "http://10.94.183.131:8081/es/search/nine";
    const ES_SCROLL_GATEWAY_URL = "http://10.94.183.131:8081/es/search/nine";
    const ES_GATEWAY_URL_PUT = "http://10.94.183.131:8081/es/operate/nine";

    const POSTTIME = 'news_posttime';
    const POSTDATE = 'news_postdate';
    const FETCHTIME = 'news_fetch_time';
    const ENTERTIME = 'es_create_time';
    ...
    const AREA_TAGS = "account_area_tags";//发布省份
    const TYPES_TAGS = "account_types_tags";//发布省份
                ...
                foreach($keywords as $keyword){
                    if($type == 'full'){
                        if($searchType == 'match' && mb_strlen($keyword) > 3){
                            $tmp['bool'] = [
                                'should' => [
                                    ["match"=>["news_title"=>["query"=>$keyword,"minimum_should_match"=>"100%"]]],
                                    ["match"=>["news_content"=>["query"=>$keyword,"minimum_should_match"=>"100%"]]],
                                ]
                            ];
                        }else{
                        ...
                        if($searchType == 'match' && mb_strlen($keyword) > 3){
                            $tmp = ["match"=>["news_title"=>["query"=>$keyword,"minimum_should_match"=>"100%"]]];
                        }else{
                            if(in_array($keyword,$fenciResult)){
                        ...
                        if($searchType == 'match' && mb_strlen($keyword) > 3){
                            $tmp = ["match"=>["news_content"=>["query"=>$keyword,"minimum_should_match"=>"100%"]]];
                        }else{
                            if(in_array($keyword,$fenciResult)){
                ...
                foreach($keywords as $keyword){
                    if($type == 'full'){
                        if($searchType == 'match' && mb_strlen($keyword) > 3){
                            $tmp['bool'] = [
                                'should' => [
                                    ["match"=>[$field=>["query"=>$keyword,"minimum_should_match"=>"100%"]]],
                                ]
                            ];
                        }else{
                            $tmp['bool'] = [
                        ...
                        if($searchType == 'match' && mb_strlen($keyword) > 3){
                            $tmp = ["match"=>[$field=>["query"=>$keyword,"minimum_should_match"=>"100%"]]];
                        }else{
...
//                        "minimum_should_match" => "60%"
    ...
    public function regionTags($region)
    {
        $regionQuery = null;
        if($region){
            if(is_string($region) || (is_array($region) && count($region) == 1)) {
                $region = is_string($region) ? $region : $region[0];
                $regionQuery = [
                    'term'=> [
                        'account_area_tags'=> $region
                    ]
                ];
            }else{
                foreach ($region as $v) {
                    ...
                    $regionQuery['bool']['should'][] = $tmp;
                }
            }
        }
        $regionQuery?$this->condition['query']['bool']['filter'][] = $regionQuery:'';
        return $this;
    }
    ...
    public function tagTags($tag)
    {
        $tagQuery = null;
        if($tag){
            if(is_string($tag) || (is_array($tag) && count($tag) == 1)) {
                $tag = is_string($tag) ? $tag : $tag[0];
                $tagQuery = [
                    'term'=> [
                        'account_types_tags'=> $tag
                    ]
                ];
            }else{
                foreach ($tag as $v) {
                    $tmp = [
                        'term' => [
                            'account_types_tags'=> $v
                        ]
                    ];
                    $tagQuery['bool']['should'][] = $tmp;
                }
            }
        }
        $tagQuery?$this->condition['query']['bool']['filter'][] = $tagQuery:'';
        return $this;
    }
...
//        if($end_time && $end_time == date('Y-m-d H:i:s',strtotime($end_time))){
    ...
    public function entertime($begin_time = null,$end_time = null){
        ...
        if($begin_time && $begin_time == date('Y-m-d H:i:s',strtotime($begin_time))){
            $this->startTime = $begin_time;
            $timeRange['range']['es_create_time']['gte'] = $begin_time;
        }
        if($end_time && $end_time == date('Y-m-d H:i:s',strtotime($end_time))){
            $this->endTime = $end_time;
            $timeRange['range']['es_create_time']['lte'] = $end_time;
        }
        $timeRange != null ? $this->condition['query']['bool']['filter'][] = $timeRange:'';
        return $this;
    }
...
//            $newFieldsArr[$field] = new \stdClass();
     ...
     * @return $this
     */
    public function group($field,$fieldAs = null,$size = null,$topHits = null,$sortBy = "es_create_time"){
        $this->size(0);
        if($topHits){
...
//            ]
    ...
    public function groupGroup($field1,$field2,$size1 = 10,$size2 = 10){
        $this->size(0);
        $tmp = [
            "group1" => [
                "terms" => [
...
//        $this->condition['aggs']['size'] = 30;
     ...
     * @return array
     */
    private function getGroupGroup(){
        $this->result = [];
        if($this->response === false) return $this->result;
        $data = $this->decodeResponse;
        if(isset($data['aggregations']["group1"])){
            foreach($data['aggregations']["group1"]['buckets'] as $v){
                $this->result[$v['key']] = isset($v['group2']["buckets"])?$v['group2']["buckets"]:[];
            }
        }
        return $this->result;
    }
    ...
    private function decorate($hit)
    {
        ...
        $result['media_CI'] = isset($result['media_CI']) ? round($result['media_CI'],1) : "";
    ...
    private function fetch(){
        ...
        $headers[] = 'Authorization:' . $this->getEsToken();
        $headers[] = 'Expect:';
        $startTime = strtotime($this->startTime) * 1000;
        $endTime = strtotime($this->endTime) * 1000;
        $url = $this->scroll?self::ES_SCROLL_GATEWAY_URL:self::ES_GATEWAY_URL;
        $params = [
            'index' => $this->index ? $this->index : Yii::$app->params['shudiEsPrefix']. CompanyService::$company_info['company_index'],
            //'index' => "new_sdxt_qingbo_test",
            'statement' => json_encode(empty($this->condition) ? new \stdClass() : $this->condition),
            'startStamp' => $startTime,
            'endStamp' => $endTime
        ];
        $this->params = json_encode($params);//var_dump($params);exit;
